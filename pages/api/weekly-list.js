...
export default withAuth(async (req, res, user, supabase) => {
  ...
  try {
    if (req.query.all) {
      const lists = await getAllWeeklyLists(supabase, user.id);
    ...
    if (req.query.id) {
      ...
      const signals    = await getSignalsForWeek(supabase, user.id, list.week_label);
      const signalMap  = {};
    ...
    const list = await getLatestWeeklyList(supabase, user.id);
