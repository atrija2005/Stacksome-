...
function scorePost(post, keywords) {
  ...
  // Keyword relevance score — must be > 0 for post to be considered
  let kwScore = 0;
  for (const kw of keywords) {
    if (title.includes(kw)) kwScore += 6;
    if (desc.includes(kw))  kwScore += 2;
    if (pub.includes(kw))   kwScore += 1;
  }

  // Engagement only amplifies relevance — zero keyword match means zero engagement boost
  // This prevents high-engagement but off-topic posts from sneaking in
  if (kwScore === 0) return 0;

  const engagement = (post._reactions || 0) + (post._comments || 0) * 3 + (post._restacks || 0) * 2;
  let score = kwScore + Math.min(engagement / 5, 40);
