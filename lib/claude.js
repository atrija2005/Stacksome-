...
Build a focused reading curriculum that takes the user from foundational understanding to genuine depth on this goal.

Rules:
- Select ONLY posts that are DIRECTLY about the stated goal — if a post is not clearly about the goal topic, do NOT include it
- If you cannot find ${count} directly relevant posts, return FEWER — never pad with off-topic or tangentially related content
- Maximum ${count} posts, minimum however many are genuinely on-topic
- Order them as a reading sequence: order 1 = best entry point, last = most advanced/specific
...
- ONLY use the substack.com subdomain format: "subdomain.substack.com" (e.g. "mattstoller.substack.com")
- Do NOT suggest custom domains — only *.substack.com URLs
...
{"publications":["subdomain1.substack.com","subdomain2.substack.com"]}`;
...
Suggest ${count} specific Substack posts that DIRECTLY serve this goal. These must be real posts you are confident exist from your training data.

For each post return:
- title: the exact post title as published
- publication_name: the newsletter name
- url: MUST use substack.com format ONLY: https://subdomain.substack.com/p/slug — derive slug from title (lowercase, hyphens, no punctuation)
- description: 2 sentences on what this post covers and why it serves the goal

Return ONLY valid JSON (no markdown, no commentary):
{"posts":[{"title":"...","publication_name":"...","url":"https://subdomain.substack.com/p/slug","description":"..."}]}

Rules:
- ONLY posts on substack.com — no custom domains, no other platforms
...
- Diverse publications — don't repeat the same newsletter`;
    ...
    const raw    = message.content[0].text.trim().replace(/^```(?:json)?\n?/, '').replace(/\n?```$/, '').trim();
    const parsed = JSON.parse(raw);
    const posts  = (parsed.posts || []).filter(p => p.title && p.url && p.publication_name && p.url.includes('substack.com'));
...
async function enrichAndFilterPool(posts, goal) {
  ...
  const MAX_SCRAPES = 25;

  // ── 1. Select which posts to scrape (priority order) ───────────────────────
  function needsScrape(p) {
    if (p._source === 'claude-suggested') return true;               // hallucination risk — always verify
    if ((p.description || '').length < 60) return true;             // thin metadata
    if (/^([A-Z][a-z]+ ){2,6}[A-Z][a-z]+$/.test(p.title || '')) return true; // slug-like title
    if ((p._profileScore || 0) < 8) return true;                    // low keyword relevance — let Claude judge
    return false;
  }
