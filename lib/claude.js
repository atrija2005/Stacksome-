...
async function rankPosts(posts, profile, likedUrls = [], skippedUrls = [], referencePosts = [], opts = {}) {
  ...
  try {
    ...
    const prompt = `You are a curriculum designer for Stacksome. The user has a specific learning goal.
...
- "tldr": 2 sentences — the concrete insight the reader walks away with (NOT a description). Start with "You'll" or a strong verb. Name the actual idea or argument.
