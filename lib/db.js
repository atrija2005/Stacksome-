 ...
 *
 * All read functions take `userId` as the second argument and scope
 * every query to that user — critical for multi-user production.
 */

// ── Publications ──────────────────────────────────────────────────────────────

async function getPublications(supabase, userId) {
...
async function getRecentPosts(supabase, userId, days = 30) {
...
async function getPostsByType(supabase, userId, days = 30) {
  const allPosts = await getRecentPosts(supabase, userId, days);
  return {
...
async function getLatestWeeklyList(supabase, userId) {
  const { data, error } = await supabase
    .from('weekly_lists')
    .select('*')
    .eq('user_id', userId)
...
async function getAllWeeklyLists(supabase, userId) {
  const { data, error } = await supabase
    .from('weekly_lists')
...
async function getSignalsForWeek(supabase, userId, weekLabel) {
  const { data, error } = await supabase
    .from('post_signals')
    .select('*')
    .eq('user_id', userId)
...
async function getAllSignals(supabase, userId) {
  const { data, error } = await supabase
    .from('post_signals')
    .select('*')
    .eq('user_id', userId);
...
async function getSignalsByUrl(supabase, userId, postUrl) {
  const { data, error } = await supabase
    .from('post_signals')
    .select('*')
    .eq('user_id', userId)
...
async function getPublicationStats(supabase, userId) {
  const { data, error } = await supabase
    .from('publications')
